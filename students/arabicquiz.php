...
<div class="container-fluid" style="width: 100%;">
    <!-- Brand and toggle get grouped for better mobile display -->
      ...
      <b>Dashboard</b>
<div>
<button class="btn "> <?php if(@$_GET['q']==0)?><a href="arabicquiz.php?q=0" style="text-decoration: none; ">Avalabile Quizes<span class="sr-only">(current)</span></a></button>
<button class="btn"> <?php if(@$_GET['q']==2)?><a href="arabicquiz.php?q=2"  style="text-decoration: none; ">History</a></button>
</div>
  </div><!-- /.container-fluid -->
...
$result = mysqli_query($dp,"SELECT * FROM quiz WHERE class_id = $class_id and subject_id = 1") or die('Error');
...
while($row = mysqli_fetch_array($result)) {
    ...
    $time = $row['time'];
	$eid = $row['eid'];
  $q12=mysqli_query($dp,"SELECT score FROM history WHERE eid='$eid' AND email='$email'" )or die('Error98');
  $rowcount=mysqli_num_rows($q12);	
if($rowcount == 0){
	echo '<tr><td>'.$c++.'</td><td>'.$title.'</td><td>'.$total.'</td><td>'.$sahi*$total.'</td><td>'.$time.'&nbsp;min</td>
	<td><b><a href="arabicquiz.php?q=quiz&step=2&eid='.$eid.'&n=1&t='.$total.'" class="pull-right btn sub1" style="margin:0px;background:#2db44a; border-radius:0%"><span class="glyphicon glyphicon-ok" aria-hidden="true"></span>&nbsp;<span class="title1"><b>Start</b></span></a></b></td></tr>';
}
else
{
...
$sn=@$_GET['n'];
$total=@$_GET['t'];
$q=mysqli_query($dp,"SELECT * FROM questions WHERE eid='$eid' AND sn='$sn' " );
...
$q=mysqli_query($dp,"SELECT * FROM options WHERE qid='$qid' " );
...
{
$eid=@$_GET['eid'];
$q=mysqli_query($dp,"SELECT * FROM history WHERE eid='$eid' AND email='$email' " )or die('Error157');
...
$q=mysqli_query($dp,"SELECT * FROM rank WHERE  email='$email' " )or die('Error157');
while($row=mysqli_fetch_array($q) )
{
$s=$row['score'];
...
if(@$_GET['q']== 2) 
{
$q=mysqli_query($dp,"SELECT * FROM history WHERE email='$email' ORDER BY date DESC " )or die('Error197');
...
$r=$row['sahi'];
$qa=$row['level'];
$q23=mysqli_query($dp,"SELECT title FROM quiz WHERE  eid='$eid' " )or die('Error208');
...
$e=$row['email'];
$s=$row['score'];
$q12=mysqli_query($dp,"SELECT * FROM user WHERE email='$e' " )or die('Error231');
